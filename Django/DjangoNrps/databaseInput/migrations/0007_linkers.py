                ...
                if domains[i].definedLinkerStop == 1:
                    if nextDomain is not None:
                        if nextDomain.definedStart is not None:
                            domains[i].definedLinkerStop = nextDomain.definedStart - 1
                        else:
                            domains[i].definedLinkerStop = None
                    elif domains[i].definedStop is not None:
                        domains[i].definedLinkerStop = domains[i].definedStop
                    else:
                        domains[i].definedLinkerStop = None
                if domains[i].pfamLinkerStop == 1:
                    if nextDomain is not None:
                        domains[i].pfamLinkerStop = nextDomain.pfamStart - 1
                    else:
                        domains[i].pfamLinkerStop = domains[i].pfamStop
                domains[i].save()
