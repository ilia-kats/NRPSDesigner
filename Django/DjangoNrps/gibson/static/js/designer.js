...
//private Variables
    ...
    /**
     * @property _hiddenFragments
     * @private
     **/
    fc._hiddenFragments = 0;
...
//public methods
	...
	fc.add = function(df)
	{
        ...
        if (ch1.f().getViewable() != 'H' || ch2.f().getViewable() != 'H' || Math.min(ch1.getOrder(), ch2.getOrder()) == 0 && Math.max(ch1.getOrder(), ch2.getOrder()) == this._hiddenFragments - 1) {
            //add the fragment
    ...
    /**
     * Add child at override
     **/
    fc.addChildAt = function(df, pos)
    {
        if (df.f().getViewable() == 'H')
            this._hiddenFragments++;
...
* @extends Container
**/
var Server = function(addCb, rmCb)
{
	this.initialize(addCb, rmCb);
}
var s = Server.prototype = new Container();
	...
	s.initialize = function(addCb, rmCb)
	{
		this._text.x = 0;
	...
	s.rmFrag = function(cf, cb)
	{
		this._showMessage('Removing Fragment...');
		var self = this;
		this._con.rmFragment(cf.id, function(data)
			{
				self._hideMessage();
...
//jQuery(window).keypress(function() {self._fc.debug();});
		...
		this._server = new Server(addCb, rmCb);
