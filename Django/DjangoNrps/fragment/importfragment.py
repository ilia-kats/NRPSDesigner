...
from forms import MetaForm, SequenceForm, PartTypeForm
...
def manual_form(request):
    ...
    seq = SequenceForm()
    ptype = PartTypeForm()
    c = RequestContext(request, {'meta': meta, 'seq': seq, 'part_type': ptype,
...
def manual_add(request):
    ...
    if request.method == 'GET':
        ...
        ptype = PartTypeForm(request.GET)
        if meta.is_valid() and seq.is_valid():
            ...
            g = Gene.add(record, 'MN', request.user)
            if ptype.is_valid() and ptype.cleaned_data['part_type'] != "":
                Annotation.add(g, 'part_type', ptype.cleaned_data['part_type'])
    ...
    if request.method == 'POST' and len(request.FILES) != 0:
        ...
        for file in files:
            ...
            for record in records:
                ...
                ids.append(g.id)
                geneData.append({'gene_name': g.name, 'id': g.id})
    ...
    if request.method == 'GET' and 'id' in request.GET:
        ...
        for r in records:
            if len(Gene.objects.filter(name=r.name, owner=request.user)) == 0:
                g = Gene.add(r, 'NT', request.user)
                data.append({'gene_name': g.name, 'id': g.pk})
