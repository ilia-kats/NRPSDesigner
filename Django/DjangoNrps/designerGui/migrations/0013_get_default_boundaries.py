...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # rewrite domain.get_start(), domain.get_stop() methods
        # so that south can actually make use of them...

        def get_start(domain, prevd=None, with_linker=True):
            if with_linker:
                if domain.definedLinkerStart is not None:
                    return domain.definedLinkerStart
                elif domain.pfamLinkerStart is not None:
                    return domain.pfamLinkerStart
                elif domain.definedStart is not None:
                    return domain.definedStart
                else:
                    return domain.pfamStart

        def get_stop(domain, nextd=None, with_linker=False):
            if domain.definedStop is not None:
                return domain.definedStop
            else:
                return domain.pfamStop

        for domain_order in orm.DomainOrder.objects.all():
            domain = domain_order.domain
            domain_order.left_boundary = get_start(domain)
            domain_order.right_boundary = get_stop(domain)
            domain_order.save()
    ...
    models = {
        ...
        u'designerGui.domainorder': {
            ...
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
            'left_boundary': ('django.db.models.fields.PositiveIntegerField', [], {}),
            'nrp': ('django.db.models.fields.related.ForeignKey', [], {'related_name': "'domainOrder'", 'to': u"orm['designerGui.NRP']"}),
            'order': ('django.db.models.fields.PositiveIntegerField', [], {}),
            'right_boundary': ('django.db.models.fields.PositiveIntegerField', [], {})
        },
        u'designerGui.nrp': {
            ...
            'uuid': ('django.db.models.fields.CharField', [], {'default': "'f6e1068c-81a3-47b9-b1a2-447bfca9cbce'", 'max_length': '36', 'db_index': 'True'})
        },
